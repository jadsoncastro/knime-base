...
//            settings.setRightIncludeAll(true);
            ...
            JoinImplementation joiner = CostModelFactory.Instance.create(settings, left, right);
            m_currentAppendTable = joiner.join(exec.createSubExecutionContext(0.9)).getSingleTable();
        }
        m_iteration += 1;
        if (continueLoop) {
            super.continueLoop();
