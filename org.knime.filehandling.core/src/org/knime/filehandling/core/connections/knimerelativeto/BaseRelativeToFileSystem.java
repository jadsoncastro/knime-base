...
public abstract class BaseRelativeToFileSystem extends BaseFileSystem<RelativeToPath> {
     ...
     * @throws IOException
     */
    protected BaseRelativeToFileSystem(final BaseRelativeToFileSystemProvider<? extends BaseRelativeToFileSystem> fileSystemProvider, final URI uri,
     ...
     */
    protected static boolean isServerContext(final WorkflowContext context) {
     ...
     */
    @Override
    public RelativeToPath getWorkingDirectory() {
        if (isWorkflowRelativeFileSystem()) {
            return getWorkflowDirectory();
        } else {
            return getPath(getSeparator());
        }
    }

    /**
     * Get the path of the current workflow in the relative-to file system.
     *
     * @return path of the current workflow in the relative-to file system.
     */
    protected abstract RelativeToPath getWorkflowDirectory();
    ...
    protected boolean isOrInCurrentWorkflowDirectory(final RelativeToPath path) {
        ...
        return toCheck.startsWith(getWorkflowDirectory());
    }
    ...
    protected boolean isCurrentWorkflowDirectory(final RelativeToPath path) {
        final Path toCheck = path.toAbsolutePath().normalize();
        return toCheck.equals(getWorkflowDirectory());
    }
